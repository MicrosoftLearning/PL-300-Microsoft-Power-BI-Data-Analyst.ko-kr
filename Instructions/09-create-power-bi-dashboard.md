...
이 랩에서는 기존 보고서를 사용하여 Power BI 서비스 **판매 모니터링** dashboard 만듭니다.
...
1. **내 작업 영역을** 열고 **판매 모니터링** dashboard 엽니다.
...
1. 오른쪽에 있는 **타일 세부 정보** 창의 **자막** 상자에 **FY2020**을 입력한 다음 **적용**을 선택합니다.
...
1. **타일 세부 정보** 창의 **기능** 섹션에서 **마지막 새로 고침 시간 표시**를 검사 다음 **적용**합니다.
...
*다음 연습에서 데이터 세트를 새로 고칩니다. 데이터 및 보고서에 따라 언제든지 임시 데이터 새로 고침을 수행하거나 일정을 설정할 수 있습니다. 그러나 예약된 새로 고침에는 이 랩에 대해 구성할 수 없는 게이트웨이가 필요합니다. 따라서 Power BI Desktop 수동 데이터 새로 고침을 수행한 다음 파일을 작업 영역에 업로드합니다.*
...
이 작업에서는 업데이트된 판매를 확인하려면 dashboard 검토합니다.
