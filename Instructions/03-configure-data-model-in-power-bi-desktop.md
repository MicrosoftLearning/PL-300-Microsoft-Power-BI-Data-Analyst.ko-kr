...
1. 계층을 만들려면 **데이터** 창에서 **범주** 열을 마우스 오른쪽 단추로 클릭한 다음 **계층 구조 만들기를** 선택합니다.

     ![계층 구조 만들기 대화 상자](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image24.png)

1. **속성** 창(**데이터** 창 왼쪽)의 **이름** 상자에서 텍스트를 **Products**로 바꿉니다.
...
1. **데이터** 창에서 두 열이 이제 폴더 내에 있음을 알 수 있습니다.
...
1. 또한 각 영업 사원의 판매액(추가 된 경우)이 테이블 총액을 초과합니다.
     ...
     *이중, 삼중 등으로 인해 다대다 관계에 대한 일반적인 관찰입니다. 지역 판매 결과의 계산. 나열된 두 번째 영업 사원인 브라이언 웰커를 생각해 보세요. 그의 판매 금액은 총 판매 금액과 같습니다. 이는 그가 영업 이사라는 사실 때문에 올바른 결과입니다. 그의 판매는 모든 지역의 판매로 측정됩니다.*
