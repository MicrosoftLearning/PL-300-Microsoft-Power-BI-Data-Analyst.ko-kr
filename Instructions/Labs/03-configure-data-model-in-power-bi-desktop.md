...
1. **D:\PL300\Labs\03-configure-data-model-in-power-bi-desktop\Starter** 폴더로 이동하여 **판매 분석** 파일을 선택합니다.
...
1. 7개의 테이블이 모두 표시되지 않으면 오른쪽으로 가로로 스크롤한 다음 테이블을 더 가깝게 끌어서 정렬하여 동시에 볼 수 있습니다.
...
1. 테이블 시각적 개체를 만들려면 **데이터** 창의 **Product** 테이블 내에서 **범주** 필드를 검사.
...
1. 테이블 시각적 개체에 4개의 Product 범주가 나열되며 각 범주의 Sales 금액이 동일하고 합계에의 금액도 동일한 것을 볼 수 있습니다.
    ...
    *문제는 테이블이 서로 다른 테이블의 필드를 기반으로 한다는 것입니다. 각 제품 범주에 해당 범주의 판매량이 표시된다는 기대가 있습니다. 그러나 이러한 테이블 간에 모델 관계가 없으므로 **Sales** 테이블은 필터링되지 않습니다. 이제 테이블 간에 필터를 전파하는 관계를 추가합니다.*
...
1. 각 테이블에서 **ProductKey** 열이 자동으로 선택되었음을 확인합니다.
    ...
    *열은 동일한 이름과 데이터 형식을 공유하기 때문에 선택되었습니다. 실제 데이터에서 이름이 다른 일치하는 열을 찾아야 할 수 있습니다.*
...
1. 이제 두 테이블 사이에 연결선이 있습니다(테이블이 나란히 배치되는지 여부는 중요하지 않음).
...
1. 모델 보기의 **데이터** 창에서 필요한 경우 **Product** 테이블을 확장하여 모든 필드를 표시합니다.

1. 계층 구조를 만들려면 **데이터** 창에서 **범주** 열을 마우스 오른쪽 단추로 클릭한 다음 **계층 만들기를** 선택합니다.

     ![계층 구조 만들기 대화 상자](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image24.png)

1. **데이터** 창의 왼쪽에 있는 **속성** 창의 **이름** 상자에서 텍스트를 **Products**로 바꿉니다.
...
1. **데이터** 창에서 두 열이 이제 폴더 내에 있음을 확인합니다.
...
1. **고급** 그룹(아래로 스크롤하여 찾아야 할 수도 있음)에 있는 **요약 기준** 드롭다운 목록에서 **평균**을 선택합니다.
    ...
    *기본적으로 숫자 열은 값을 함께 합산하여 요약됩니다. 이 기본 동작은 속도를 나타내는 **Unit Price**와 같은 열에 적합하지 않습니다. 기본 요약을 평균으로 설정하면 의미 있는 결과가 생성됩니다.*
...
1. **Sales \| OrderDate** 필드를 확장하면 날짜 계층 구조가 표시되는 것을 볼 수 있습니다.
 ...
 이제 이 자동 동작을 끕니다. **Power BI Desktop DAX 계산 만들기 랩에서** DAX를 사용하여 날짜 테이블을 만들고 Adventure Works 일정을 정의하도록 구성합니다.
...
1. **빠른 측정값** 창의 **데이터** 창에서 **Sales** 테이블을 확장합니다.
...
1. 측정값의 이름을 바꾸려면 해당 측정값을 마우스 오른쪽 단추로 클릭하고 **이름 바꾸기**를 선택한 다음 이름을 Profit으로 바꿉 **니다**.
...
1. Power BI Desktop 보고서 보기의 **데이터** 창에서 다음 두 필드를 검사 **테이블** 시각적 개체를 만듭니다.
...
1. 보고서 뷰로 전환한 다음 판매 값이 아직 변경되지 않은 것을 확인합니다.
    ...
    *이제 이 문제는 **Salesperson** 테이블과 **Sales** 테이블 사이에 두 개의 필터 전파 경로가 있다는 사실과 관련이 있습니다. 이 모호성은 "최소 수의 테이블" 평가를 기반으로 내부적으로 해결됩니다. 분명히 하려면 이러한 유형의 모호성을 사용하여 모델을 디자인해서는 안 됩니다. 이 문제는 이 랩의 뒷부분과 **Power BI Desktop 랩에서 DAX 계산 만들기**가 완료될 때 부분적으로 해결됩니다.*
...
1. 또한 각 영업 사원의 판매액(추가 된 경우)이 테이블 총액을 초과합니다.
     ...
     *지역 판매 결과의 이중, 삼중 등으로 인해 다대다 관계에 대한 일반적인 관찰입니다. 나열된 두 번째 영업 사원인 브라이언 웰커를 생각해 보세요. 그의 판매 금액은 총 판매 금액과 같습니다. 이는 그가 영업 이사라는 사실 때문에 올바른 결과입니다. 그의 판매는 모든 지역의 판매로 측정됩니다.*

     *다대다 관계가 현재 작동 중이지만 이제 영업 사원이 수행한 매출을 분석할 수 없습니다(관계가 비활성 상태이기 때문). Power BI Desktop 랩에서 **DAX 계산 만들기** 랩에서 영업 사원에게 할당된 판매 지역(성능 분석용)에서 만든 매출을 분석할 수 있는 계산 테이블을 도입하면 관계를 다시 활성화할 수 있습니다.*
...
이 작업에서는 Targets 테이블에 대한 관계를 만듭니**다.**
 ...
 *이제 판매 및 대상을 시각화할 수 있지만 두 가지 이유로 주의해야 합니다. 첫째, 기간에 대한 필터가 없으므로 대상에 향후 대상 금액도 포함됩니다. 둘째, 대상은 가산적이지 않으므로 합계를 표시해서는 안 됩니다. 시각적 개체의 서식을 지정하여 사용하지 않도록 설정하거나 계산 논리를 사용하여 제거할 수 있습니다. 두 번째 방법은 두 개 이상의 영업 사원이 필터링될 때 BLANK를 반환하는 **Power BI Desktop 랩에서 고급 DAX 계산 만들기에서** 대상 측정값을 만들어 수행합니다.*

### **완료**

이번 작업에서는 랩을 마무리합니다.

Power BI Desktop 파일을 저장하고 쿼리를 적용하라는 메시지가 표시되면 **나중에** 적용을 선택합니다.
