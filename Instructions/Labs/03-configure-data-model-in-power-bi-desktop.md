...
이 작업에서는 모델 관계를 만듭니다. 이 파일은 이전 랩의 테이블 간 관계를 식별하지 않도록 구성되었습니다. 기본 설정은 아니지만 모델에 대한 올바른 관계를 만드는 추가 작업을 방지하는 것이 좋습니다.

*중요: 랩은 약식 표기법을 사용하여 필드를 참조합니다. 제품 \| 범주**는 다음과 **같습니다. 이 예제에서 **Product**는 테이블 이름이고 **범주**는 필드 이름입니다.*

1. Power BI Desktop의 왼쪽에서 **모델** 뷰 아이콘을 선택합니다.

     ![그림 1](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image9.png)

1. 7개의 테이블이 모두 표시되지 않으면 오른쪽으로 가로로 스크롤한 다음 테이블을 더 가깝게 끌어서 정렬하여 모두 동시에 볼 수 있습니다.
...
1. 모든 테이블 필드를 보려면 데이터 창에서 **빈 영역을 마우스 오른쪽 단추로 클릭한 다음 모두** 확장을 선택합니다**.**

1. 테이블 시각적 개체를 만들려면 데이터 창의 **Product** 테이블 내부에서 **범주** 필드를 검사**.**

1. 테이블에 다른 열을 추가하려면 데이터 창에서 **Sales Sales** \| 필드를 검사**.**
...
1. 각 테이블에서 **ProductKey** 열이 자동으로 선택되었음을 확인합니다.
    ...
    *열이 동일한 이름과 데이터 형식을 공유하기 때문에 선택되었습니다. 실제 데이터에서 이름이 다른 일치하는 열을 찾아야 할 수 있습니다.*
...
1. 확인을 선택하고 **관계 관리** 창에서 **새 관계가 나열된 것을 확인하고 닫기를** 선택합니다**.**

1. 이제 두 테이블 사이에 커넥터가 있습니다(테이블이 나란히 배치되는지 여부는 중요하지 않음).
    1. 1** 및 **(*)** 표시기가 나타내는 **카드inality를 해석할 수 있습니다.
...
1. 다른 기술을 **사용하여 새 관계를 만들려면 Reseller** 테이블에서 ResellerKey** 열을 Sales** 테이블의 **ResellerKey** 열로 **끌어**옵니다.
...
1. 보고서 보기에서 테이블 시각적 개체가 업데이트되어 각 제품 범주에 대해 서로 다른 값을 표시합니다.
    ...
    **제품** 테이블에 적용된 필터가 이제 **판매** 테이블로 전파됩니다.
     ...
     ![새 관계로 범주 및 판매 번호를 업데이트했습니다.](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image20.png)
...
이 작업에서는 Product** 테이블을 구성**합니다.

1. 모델 보기의 **데이터** 창에서 필요한 경우 Product** 테이블을 확장**하여 모든 필드를 표시합니다.

1. 계층을 만들려면 데이터 창에서 **범주** 열을 마우스 오른쪽 단추로 클릭한 **다음 계층 만들기를** 선택합니다**.**

     ![계층 구조 만들기 대화 상자](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image24.png)

1. 속성 창(데이터** 창 왼쪽)의 **이름** 상자에서 **텍스트를 Products로 **바꿉니다**.** ** 
...
1. 계층 구조 디자인을 완료하려면 **수준 변경 내용 적용**을 선택합니다.
    ...
    *팁: 수준 변경 내용** 적용을 선택하는 **것을 잊지 마세요. 이 단계를 간과하는 것은 일반적인 실수입니다.*

1. **데이터** 창에서 제품 계층 구조를 확인 **** 합니다.
...
1. 열을 표시 폴더로 구성하려면 데이터 창에서 **먼저 배경색 서식** 열을 선택합니다**.**
...
1. **데이터** 창에서 두 열이 이제 폴더 안에 있습니다.
    
   *표시 폴더는 특히 많은 필드로 구성된 테이블의 경우 테이블을 정리하는 좋은 방법입니다. 논리적 프레젠테이션일 뿐입니다.*

     ![그림 349](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image30.png)

## **지역 테이블 구성**

이 작업에서는 지역** 테이블을 구성**합니다.
...
이 작업에서는 Sales** 테이블을 구성**합니다.

1. **Sales** 테이블에서 **Cost** 열을 선택합니다.

1. 속성 창의 **설명** 상자에 표준 비용에* 따라 다음*** 을 입력합니다.**

    *설명은 테이블, 열, 계층 또는 측정값에 적용할 수 있습니다. **보고서 작성자가 필드 위로 커서를 가져가면 데이터** 창에서 설명 텍스트가 도구 설명에 표시됩니다.*
...
1. **속성** 창에서 **숨김** 속성을 **켬**으로 밉니다.
    ...
    *Power BI Desktop** 랩에서 **DAX 계산 만들기의 계산에 **SalesOrderNumber**를 사용합니다.*
...
1. **데이터** 창에서 다음을 확인합니다.
     ...
     - SalespersonRegion** 테이블은 **표시되지 않습니다. 모든 필드가 숨겨져 있기 때문입니다.
...
1. **판매 \| OrderDate** 필드를 확장하면 날짜 계층 구조가 표시되는 것을 볼 수 있습니다.
    ...
    ***Targets \| TargetMonth** 필드는 비슷한 계층 구조를 제공합니다. 이러한 계층 구조는 사용자가 만들지 않았습니다. 자동으로 생성되었습니다. 그러나 문제가 있습니다. Adventure Works 회계 연도는 매년 7월 1일에 시작됩니다. 그러나 자동으로 생성된 이러한 날짜 계층에서 날짜 계층 구조 연도는 매년 1월 1일에 시작됩니다.*

     ![그림 359](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image40.png)

 이제 이 자동 동작을 해제합니다. Power BI Desktop** 랩의 **DAX 계산 만들기에서 DAX를 사용하여 날짜 테이블을 만들고 Adventure Works 달력을 정의하도록 구성합니다.
...
1. **데이터** 창에서 날짜 계층을 더 이상 사용할 수 없습니다.
...
이 작업에서는 이익과 이익률을 계산하는 두 가지 빠른 측정값을 만듭니다.

*빠른 측정은 계산 수식을 만듭니다. 간단하고 일반적인 계산을 위해 쉽고 빠르게 만들 수 있습니다. Power BI Desktop** 랩에서 DAX 계산 만들기에서 **이 도구를 사용하지 않고 측정값을 만듭니다.*

1. **데이터** 창에서 Sales** 테이블을 마우스 오른쪽 단추로 **클릭한 다음 새 빠른 측정값**을 선택합니다**.
...
1. **빠른 측정값** 창의 데이터** 창에서 **Sales** 테이블을 확장**합니다.
...
1. 데이터** 창의 **Sales** 테이블 내에서 **새 측정값을 확인합니다.
    1. *측정값은 계산기 아이콘을 사용합니다.*

     ![그림 370](Linked_image_Files/03-configure-data-model-in-power-bi-desktop_image50.png)

1. 측정값의 이름을 바꾸려면 해당 측정값을 마우스 오른쪽 단추로 클릭하고 이름 바꾸기를 선택한 **다음, Profit**으로 **이름을 바꿉**니다.
...
1. Power BI Desktop의 보고서 보기에서 데이터 창에서 **다음 두 필드를 검사 테이블** 시각적 개체를 **만듭니**다.
...
1. 보고서 보기로 전환한 다음 시각적 개체가 업데이트되지 않은 것을 확인합니다. Michael Blythe의 판매 결과는 변경되지 않았습니다.

1. 다시 모델 뷰로 전환한 다음 **Salesperson** 테이블에서 관계 필터 방향(화살촉)을 따릅니다.

    *Salesperson** 테이블이 **Sales** 테이블을 필터링하는 것을 **고려합니다. 또한 SalespersonRegion** 테이블을 필터링**하지만 필터**를 Region** 테이블로 전파하여 계속하지 않습니다(화살촉이 잘못된 방향을 가리키고 있습니다).*
...
1. **양방향** 검사에서 보안 필터 적용을 선택한 다음 확인을 선택합니다.
...
1. 보고서 뷰로 전환한 다음 판매 값이 아직 변경되지 않은 것을 확인합니다.
    ...
    *이제 이 문제는 Salesperson**과 Sales** 테이블 사이에 두 개의 가능한 필터 전파 경로가 **있다는 사실과 **관련이 있습니다. 이 모호성은 "최소 테이블 수" 평가에 따라 내부적으로 해결됩니다. 분명히 하려면 이러한 유형의 모호성을 사용하여 모델을 디자인해서는 안 됩니다. 이 문제는 이 랩의 뒷부분과 Power BI Desktop** 랩에서 DAX 계산 만들기가 완료**될 때 부분적으로 해결됩니다.*

1. 모델 보기로 전환하여 브리징 테이블을 통해 필터 전파를 강제 적용합니다. Salesperson**과 **Sales** 테이블 간의 **관계를 편집(두 번 클릭)합니다.

1. **관계 편집 창에서 이 관계**** 만들기 검사 **상자를 검사 해제하고 확인을** 선택합니다**.
...
1. 보고서 보기로 전환한 다음, Michael Blythe의 매출은 이제 거의 2,200만 달러에 달합니다.
...
1. 또한 각 영업 사원의 판매액(추가 된 경우)이 테이블 총액을 초과합니다.
     ...
     *지역 판매 결과의 이중, 삼중 등으로 인해 다대다 관계에 대한 일반적인 관찰입니다. 나열된 두 번째 영업 사원인 브라이언 웰커를 생각해 보십시오. 그의 판매 금액은 총 판매 금액과 같습니다. 그가 영업 이사라는 사실 때문에 올바른 결과입니다. 그의 판매는 모든 지역의 판매로 측정됩니다.*

     *다대다 관계가 작동하는 동안에는 영업 사원이 수행한 매출을 분석할 수 없습니다(관계가 비활성 상태이기 때문). Power BI Desktop** 랩의 DAX 계산 만들기 랩에서 영업 사원에게 할당된 판매 지역(성능 분석용)** 에서 만든 매출을 분석할 수 있는 계산 테이블을 도입할 때 관계를 다시 활성화할 수 있습니다.*
 ...
 *이제 판매 및 대상을 시각화할 수 있지만 두 가지 이유로 주의해야 합니다. 첫째, 기간에 대한 필터가 없으므로 대상에는 향후 목표 금액도 포함됩니다. 둘째, 대상은 가산적이지 않으므로 합계를 표시해서는 안 됩니다. 시각적 개체의 서식을 지정하여 사용하지 않도록 설정하거나 계산 논리를 사용하여 제거할 수 있습니다. 두 번째 방법은 둘 이상의 영업 사원이 필터링될 때 BLANK를 반환하는 Power BI Desktop** 랩의 고급 DAX 계산 만들기에서 대상 측정값**을 만들어서 수행합니다.*
