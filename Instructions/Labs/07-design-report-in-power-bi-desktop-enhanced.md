...
1. **열기** 창에서 D:\Allfiles\Labs\07-design-report-in-power-bi-desktop-enhanced\Starter** 폴더로 이동하고 **Sales Analysis** 파일을 엽니다**.
...
이 작업에서는 연도 및 지역** 슬라이서를 동기화**하여 **Power BI Desktop** 랩에서 보고서 디자인에서 **만든 보고서의 개발을 계속합니다.**
...
1. **[효과] > 배경**색 속성을 밝은 회색 음영(예: 흰색, 20% 더 어둡게 *)으로 *설정합니다.
...
1. 이 기준이 **되는 필드는 무엇인가요?** 드롭다운 목록에서 제품 \| 서식 배경색 형식**을 \| 선택한 **다음 **확인**합니다.
...
이 연습에서는 보고서 사용자가 표시할 시각적 개체 유형을 선택할 수 있도록 단추가 있는 내 성능** 페이지를 향상시킵**니다. 디자인을 완료하면 페이지는 다음과 같은 모습이 됩니다.
...
이 작업에서는 매월 각 판매/대상 시각적 개체를 표시하는 책갈피를 두 개 추가합니다.
...
1. 단추를 선택한 다음 서식 단추 창에서 **단추를** 선택하고 **스타일** 섹션을 확장하고 **텍스트** 속성을 켜**기로 **설정합니다**.**
...
1. 보고서를 게시하려면 [선택]을 선택합니다****.
    1. 의미 체계 모델을 바꾸라는 메시지가 표시되면 바꾸기**를 선택합니다**.
...
1. 드릴스루 기능을 테스트하려면 개요** 페이지 > **범주**별 수량 시각적 개체로 이동합니다**. 그런 다음 의류 표시줄을 마우스 오른쪽 단추로 **클릭하고 제품 세부 정보** 드릴스루 \| 를 선택합니다**.**
...
1. **내 실적** 페이지를 선택합니다.
    ...
    *각 단추를 선택한 다음 다른 시각적 개체가 표시되는 것을 확인합니다.*
