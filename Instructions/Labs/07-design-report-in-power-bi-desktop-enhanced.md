...
이 작업에서는 Power BI에 로그인하여 랩에 대한 환경을 설정합니다.
...
*중요: 이전 랩에서 계속 진행 중이고 랩을 성공적으로 완료한 경우 이 작업을 완료하지 마세요. 대신 다음 작업으로 건너뜁니다.*
...
이 작업에서는 **연도** 및 **지역** 슬라이서를 동기화하여 **Power BI Desktop 보고서 디자인 랩에서 만든 보고서의** 개발을 계속합니다.
...
1. 보고서 페이지의 왼쪽 위에 화살표 단추가 표시된 것을 확인합니다.
    ...
    *필드가 드릴스루 웰/영역에 추가되면 화살표 단추가 자동으로 추가됩니다. 보고서 사용자가 드릴스루한 페이지로 다시 이동할 수 있습니다.*
...
1. **효과 > 배경**색 속성을 *흰색, 20% 더 어두운* 등 밝은 회색 음영으로 설정합니다.
...
1. 드릴스루 기능을 테스트하려면 **개요** 페이지 > **범주별 수량** 시각적 개체로 이동합니다. 그런 다음 **의류** 표시줄을 마우스 오른쪽 단추로 클릭하고 **제품 세부 정보 드릴스루를 \|** 선택합니다.
