...
이 작업에서는 Salesperson** 계산 테이블(Sales**와 직접 관계)을 **만듭니**다.
...
2. 계산을 만들거나 편집할 때 리본 바로 아래에 표시되는 수식 입력줄에서 **Salesperson =** 를 입력하고 **Shift + Enter** 키를 누르고, **'Salesperson (Performance)'** 를 입력한 다음 **Enter** 키를 누릅니다.
    ...
    *편의를 위해 이 랩의 모든 DAX 정의를 D:\Allfiles\Labs\04-create-dax-calculations-in-power-bi-desktop\Assets\Snippets.txt** 있는 **코드 조각 파일에서 복사할 수 있습니다.*
     ...
     *이 테이블 정의는 Salesperson(성능)** 테이블의 **복사본을 만듭니다. 데이터만 복사되지만 표시 유형, 서식 등과 같은 모델 속성은 복사되지 않습니다.*

     *팁: 특히 수식이 길고 복잡한 경우 직관적이고 읽기 쉬운 형식으로 수식을 작성하려면 "공백"(즉, 캐리지 리턴 및 탭)을 입력하는 것이 좋습니다. 캐리지 리턴을 입력하려면 Shift+Enter를 누릅니 **다**. "공백"은 선택 사항입니다.*

1. **데이터** 창에서 테이블 아이콘은 파란색 음영(계산된 테이블을 나타냅니다)입니다.

    ![그림 10](Linked_image_Files/05-create-dax-calculations-in-power-bi-desktop_image11.png)

    *참고: 계산된 테이블은 테이블을 반환하는 DAX 수식을 사용하여 정의됩니다. 계산된 테이블은 값을 구체화하고 저장하기 때문에 데이터 모델의 크기가 증가한다는 것을 이해하는 것이 중요합니다. 새(미래) 날짜 값이 테이블에 로드될 때 이 데이터 모델의 경우처럼 수식 종속성이 새로 고쳐질 때마다 다시 계산됩니다.*
...
1. **속성** 창의 **설명** 상자에 **판매와 관련된 영업 직원**
    ...
    *사용자가 테이블 또는 필드 위에 커서를 **놓으면 설명이 데이터** 창에 도구 설명으로 표시되는 것을 기억할 수 있습니다.*
...
1. 날짜 값의 열을 확인합니다.
    ...
    *표시된 날짜는 미국 지역 설정(즉, mm/dd/yyyy)을 사용하여 형식이 지정됩니다.*
...
1. 수식 표시줄에서 다음을 입력(하거나 코드 조각 파일에서 복사)한 후 **Enter** 키를 누릅니다.
    ...
    *계산 열은 먼저 열 이름을 입력한 다음 등호(=) 뒤에 단일 값 결과를 반환하는 DAX 수식을 입력하여 생성됩니다. 열 이름이 테이블에 아직 있을 수 없습니다.*
...
1. 데이터 창의 **날짜** 테이블 내부에서 **연도** 필드를 행 웰/영역으로 **끕**니다**.**
...
1. 행렬 시각적 개체의 오른쪽 위(또는 시각적 개체의 위치에 따라 아래쪽)에서 포크된 이중 화살표 아이콘(모든 연도를 한 수준 아래로 확장)을 선택합니다.
...
1. 보고서 뷰로 다시 전환합니다. **데이터** 창에서 월** 필드가 **선택되어 있는지 확인합니다(선택하면 진한 회색 배경이 됩니다).
...
1. 데이터 오른쪽 창에서 **날짜** 테이블을 선택하고**, 연도** 열에서 오른쪽을 **선택하고, 계층** 만들기를 선택합니다**.** 

1. 오른쪽 선택 및 **이름 바꾸기를 통해 **새로 만든 계층의 이름을 Fiscal**으로 바꿉니다**.


1. 데이터 창에서 **해당 필드를 선택하고, 마우스 오른쪽 단추를 클릭하고, 계층 구조 회계에 추가를 선택하여 **다음 두 개의 다시 기본 필드를 회계** 계층에** -> **추가합니다.**
...
1. Power BI Desktop 파일을 저장합니다.
    ...
    *이제 Power BI Desktop은 이 테이블이 날짜(시간)를 정의한다는 것을 이해합니다. 시간 인텔리전스 계산에 의존할 때는 중요합니다. Power BI Desktop** 랩에서 고급 DAX 계산 만들기에서 **시간 인텔리전스 계산을 사용합니다.*

    *날짜 테이블에 대한 이 디자인 방법은 데이터 원본에 날짜 테이블이 없는 경우에 적합합니다. 데이터 웨어하우스가 있는 경우 데이터 모델의 날짜 논리를 "재정의"하는 대신 날짜 차원 테이블에서 날짜 데이터를 로드하는 것이 적절합니다.*

## **간단한 측정값 만들기**

이 작업에서는 단순 측정값을 만듭니다. 간단한 측정값은 단일 열의 값을 집계하거나 테이블의 행 개수를 계산합니다.

1. 보고서 보기의 **2페이지 데이터** 창에서 **Sales \| Unit Price** 필드를 행렬 시각적 개체로 끕**니다.**
...
1. **Unit Price**의 아래쪽 화살표를 선택한 다음, 사용 가능한 메뉴 옵션을 확인합니다.
    ...
    *표시되는 숫자 열을 사용하면 보고서 디자인 타임에 보고서 작성자가 열 값을 요약하는 방법을 결정할 수 있습니다(또는 그렇지 않음). 부적절한 보고가 발생할 수 있습니다. 그러나 일부 데이터 모델러는 이러한 열을 숨기고 측정값에 정의된 집계 논리를 노출하도록 선택하는 것을 좋아하지 않습니다. 이제 이 랩에서 사용할 접근 방식입니다.*
...
1. 값** 웰**에서 Avg Price** 필드의 **상황에 맞는 메뉴를 열고 집계 기술을 변경할 수 없습니다.
...
1. 보고서 보기에서 1**페이지를 선택하고 **테이블 시각적 개체를 검토하여 대상** 열의 합계를 **확인합니다.
...
1. **Targets \| Target** 열의 이름을 **Targets \| TargetAmount**로 바꿉니다.
    ...
    *팁: 보고서 보기에서 열 이름을 바꾸는 방법에는 여러 가지가 있습니다. 데이터 창에서 **열을 마우스 오른쪽 단추로 클릭한 다음 이름 바꾸기**를 선택**하거나 열을 두 번 클릭하거나 F2** 키를 누릅니 **** 다.*
...
1. 모든 열과 행을 확인할 수 있도록 테이블 시각적 개체의 크기를 조정합니다.
    ...
    *모든 영업 사원이 대상을 충족하지 않는 것처럼 보이지만 테이블 시각적 개체는 아직 특정 기간별로 필터링되지 않습니다. Power BI Desktop** 랩에서 보고서 디자인에서 **사용자가 선택한 기간별로 필터링하는 판매 성과 보고서를 생성합니다.*
...
*Power BI Desktop** 랩에서 고급 DAX 계산 만들기 랩에서 DAX를 **사용하여 고급 계산을 통해 데이터 모델을 향상시킵니다.*
