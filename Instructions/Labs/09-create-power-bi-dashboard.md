...
1. Power BI 서비스에서 **영업 분석** 보고서를 엽니다.
...
1. 파란색 상자의 질문 및 답변 상자 아래에서 제안된 질문 중 하나를 선택하고 응답을 검토합니다.

1. Q&A 상자에서 모든 텍스트를 제거하고 다음을 입력합니다. **Sales YTD**

1. **(Blank)** 응답을 확인합니다.
    
    ***Power BI Desktop에서 고급 DAX 계산 만들기** 랩에서는 **Sales YTD** 측정을 추가했습니다. 이 측정은 시간 인텔리전스 식이므로 결과를 생성하려면 **날짜** 테이블의 필터가 필요합니다.*
...
1. **이미지 타일 추가** 창의 **URL** 상자에 **D:\Allfiles\Resources\AdventureWorksLogo_DataURL.txt** 파일에 있는 전체 URL을 입력한 다음 **적용**을 선택합니다.
