...
이 랩에서는 판매원이 할당된 지역에 대한 판매 데이터만 분석할 수 있도록 행 수준 보안을 적용합니다. 다음 방법에 대해 설명합니다.

- Power BI에서 동적 행 수준 보안을 구현합니다.
- USERPRINCIPALNAME()을 사용하여 역할을 만들고 테스트합니다.
...
1. 필터를 할당하려면 **Salesperson(성능)** 테이블을 선택한 다음 **규칙** 섹션에서 **DAX 편집기**로 전환을 선택합니다.
...
1. 보고서 페이지 위의 빨간색 배너에서 테스트 보안 컨텍스트를 설명하는 것을 볼 수 있습니다.
...
1. 테스트를 중지하려면 빨간색 배너의 오른쪽에서 **보기 중지**를 선택합니다.
...
이 랩에서는 필요하지 않지만, Power BI 보고서를 저장하도록 선택할 수 있습니다. 

1. Microsoft Edge 브라우저 창을 닫습니다.
1. Power BI Desktop에서 왼쪽 위 모서리에 있는 **"파일"** 메뉴로 이동하여 **"다른 이름으로 저장"** 을 선택합니다. 
