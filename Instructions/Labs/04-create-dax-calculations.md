...
1. `Salesperson` 테이블에서 다음 열을 여러 개 선택한 다음 숨깁니다(**숨김** 속성을_예_로 설정).
...
1. `Date` 테이블에서`MonthKey` 열을 숨깁니다(**숨김**을_예_로 설정).
...
1. **날짜 테이블로 표시** 창에서**날짜 테이블로 표시** 속성을_예_로 밀어 선택합니다.
...
1. `Targets | Target` 열의 이름을_TargetAmount_로 바꾸세요.
