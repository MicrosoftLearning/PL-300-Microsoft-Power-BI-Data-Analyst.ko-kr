...
1. **열기** 창에서 **D:\PL300\Labs\04-create-dax-calculations-in-power-bi-desktop\Starter** 폴더로 이동하여 **Sales Analysis** 파일을 엽니다.
...
2. 계산을 만들거나 편집할 때 리본 바로 아래에 표시되는 수식 입력줄에서 **Salesperson =** 를 입력하고 **Shift + Enter** 키를 누르고, **'Salesperson (Performance)'** 를 입력한 다음 **Enter** 키를 누릅니다.
     ...
     *이 테이블 정의는 **Salesperson(성능)** 테이블의 복사본을 만듭니다. 데이터만 복사합니다. 그러나 표시 유형, 서식 지정 등과 같은 모델 속성은 복사됩니다. 복사되지 않습니다.*

     *팁: 특히 수식이 길고 복잡한 경우 직관적이고 읽기 쉬운 형식으로 수식을 작성하기 위해 "공백"(즉, 캐리지 리턴 및 탭)을 입력하는 것이 좋습니다. 캐리지 리턴을 입력하려면 **Shift+Enter**를 누릅니다. "공백"은 선택 사항입니다.*

1. **데이터** 창에서 테이블 아이콘이 파란색 음영(계산 테이블 표시)임을 확인합니다.

    ![그림 10](Linked_image_Files/05-create-dax-calculations-in-power-bi-desktop_image11.png)

    *참고: 계산된 테이블은 테이블을 반환하는 DAX 수식을 사용하여 정의됩니다. 계산된 테이블은 값을 구체화하고 저장하기 때문에 데이터 모델의 크기가 증가한다는 것을 이해하는 것이 중요합니다. 새 날짜 값이 테이블에 로드될 때처럼 수식 종속성이 새로 고쳐질 때마다 다시 계산됩니다.*

    Power Query 원본 테이블과는 달리, 계산된 테이블을 사용하여 외부 데이터 원본의 데이터를 로드할 수 없습니다. 데이터 모델에 이미 로드된 항목에 따라 데이터만 변환할 수 있습니다.

1. 모델 보기로 전환하면 **Salesperson** 테이블을 사용할 수 있습니다(테이블을 찾으려면 보기를 다시 설정해야 할 수 있음).
...
1. **속성** 창의 **설명** 상자에 **판매와 관련된 영업 직원**
    ...
    *사용자가 테이블 또는 필드 위에 커서를 놓으면 설명이 **데이터** 창에 도구 설명으로 표시된다는 점을 기억해 볼 수 있습니다.*
...
1. 수식 표시줄에서 다음을 입력(하거나 코드 조각 파일에서 복사)한 후 **Enter** 키를 누릅니다.
    ...
    *계산 열은 먼저 열 이름을 입력한 다음 같음 기호(=) 뒤에 단일 값 결과를 반환하는 DAX 수식을 차례로 입력하여 만듭니다. 열 이름이 테이블에 아직 있을 수 없습니다.*
