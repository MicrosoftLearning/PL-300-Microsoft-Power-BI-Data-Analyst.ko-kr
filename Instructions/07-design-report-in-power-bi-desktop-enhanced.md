---
lab:
  title: Power BI Desktop 보고서 향상
  module: 7 - Create Reports
---


# Power BI Desktop 보고서 향상

**이 랩의 예상 완료 시간은 45분입니다.**

이 랩에서는 고급 디자인 기능을 **사용하여 판매 분석을 향상시킵니다** .
...
*중요: 이전 랩에서 계속 진행하는 경우(해당 랩을 성공적으로 완료한 경우) 이 작업을 완료하지 마세요. 대신 다음 작업으로 건너뜁니다.*
...
1. **열기** 창에서 **D:\PL300\Labs\07-design-report-in-power-bi-desktop-enhanced\Starter** 폴더로 이동하고 **Sales Analysis** 파일을 엽니다.
...
이 작업에서는 **연도** 및 **지역** 슬라이서를 동기화하고 **Power BI Desktop 보고서 디자인 랩에서 만든 보고서의** 개발을 계속합니다.

1. Power BI Desktop의 **개요** 페이지에서 **Year** 슬라이서를 **FY2018**로 설정합니다.

1. **My Performance** 페이지로 이동하면 **Year** 슬라이서가 다른 값으로 설정되어 있음을 알 수 있습니다.
    
    *슬라이서가 동기화되지 않으면 데이터 허위 진술과 보고서 사용자의 불만에 기여할 수 있습니다. 이제 보고서 슬라이서를 동기화합니다.*
...
1. **슬라이서 동기화** 페이지를 닫으려면 창의 오른쪽 위에 있는 **X**를 선택합니다.
...
이 작업에서는 새 페이지를 만들고 드릴스루 페이지로 구성합니다.
...
1. 보고서 페이지의 왼쪽 위에 화살표 단추가 표시된 것을 확인합니다.
    ...
    *필드가 드릴스루 웰/영역에 추가되면 화살표 단추가 자동으로 추가됩니다. 이를 통해 보고서 사용자는 드릴스루한 페이지로 다시 이동할 수 있습니다.*
...
1. **효과 > 배경**색 속성을 *흰색, 20% 더 어두운* 등 회색의 밝은 음영으로 설정합니다.
...
## **연습 3: 조건부 Formatting 추가**

이 연습에서는 조건부 서식을 사용하여 드릴스루 페이지를 향상시킵니다. 디자인을 완료하면 페이지는 다음과 같은 모습이 됩니다.
...
### **작업 1: 조건부 Formatting 추가**

이 작업에서는 조건부 서식을 사용하여 드릴스루 페이지를 향상시킵니다.

1. 테이블 시각적 개체를 선택합니다. 시각화 창에서 **이익률** 값의 아래쪽 화살표를 선택한 다음 **조건부 서식 아이콘을 \|** 선택합니다.
...
1. 다음과 같이 두 번째 규칙(녹색 원)을 구성한 다음 **확인을** 선택합니다.
    
    Profit Margin 값이 0 미만인 경우 빨간Color 다이아몬드를 표시하고 값이 0보다 크거나 같으면 녹Color 원을 표시하는 것으로 규칙을 해석할 수 있습니다.
...
1. **이 기준은 무엇인가요?** 드롭다운 목록에서 **제품 \| 서식 \| 배경색 형식**을 선택한 다음 **확인을** 선택합니다.
...
이 연습에서는 보고서 사용자가 표시할 시각적 개체 유형을 선택할 수 있도록 단추를 사용하여 **내 성능** 페이지를 향상시킵니다. 디자인을 완료하면 페이지는 다음과 같은 모습이 됩니다.
...
이 작업에서는 매월 판매/대상 시각적 개체 각각을 표시하는 책갈피 두 개를 추가합니다.
...
1. 책갈피를 편집하려면 **책갈피** 창에서 책갈피 위에 커서를 놓고 줄임표를 선택한 다음 **데이터를 선택합니다.**
...
1. 책갈피를 업데이트하려면 줄임표를 **다시 선택한 다음** 업데이트를 선택합니다.
...
1. 두 시각적 개체가 다중 카드 시각적 개체 아래에 배치되고 서로 완전히 중첩되도록 크기와 위치를 조정합니다.
    ...
    *가려진 시각적 개체를 선택하려면 선택 창에서 **선택합니다** .*
...
이 작업에서는 두 개의 단추를 추가하고 각각에 책갈피 작업을 할당합니다.
...
1. 단추를 선택한 다음 **서식 단추** 창에서 **단추를** 선택하고 **스타일** 섹션을 확장한 다음 **Text** 속성을 **켜**기로 설정합니다.

     ![그림 126](Linked_image_Files/08-design-report-in-power-bi-desktop-enhanced_image49b.png)

1. **텍스트** 섹션을 확장한 다음 **텍스트** 상자에 **가로 막대형 차트**를 입력합니다.

1. **채우기** 섹션을 확장한 다음, 보완 색을 사용하여 채우기 색을 설정합니다.

1. **단추를** 선택하고 **작업** 속성을 **켜**기로 설정합니다.
...
이 연습에서는 Power BI 서비스 보고서를 살펴봅니다.

### **작업 1: 보고서 살펴보기**

이 작업에서는 Power BI 서비스 보고서를 탐색합니다.

1. Microsoft Edge 브라우저 창에서 Power BI 서비스 > **내 작업 영역**으로 이동한 다음 **, 판매 분석** 보고서를 선택합니다.

1. 드릴스루 기능을 테스트하려면 **개요** 페이지 > **범주별 수량** 시각적 개체로 이동합니다. 그런 다음 의류 표시줄**을** 마우스 오른쪽 단추로 클릭하고 **제품 세부 정보 드릴스루를 \|** 선택합니다.
...
1. 원본 페이지로 돌아가려면 페이지의 왼쪽 위 모서리에서 화살표 단추를 선택합니다.
...
작업 영역으로 돌아가려면 창 웹 페이지의 배너에서 **내 작업 영역을** 선택합니다.
